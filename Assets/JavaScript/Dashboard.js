...
function confirm(action, var1, var2){
    if(action){
        ...
        var confirm_function = '<a href="Dashboard.php"><button class="confirm" onclick="Remove_Report(\'' + reportId + '\',\''+ reportedMessageId +'\')">Valider</button></a>' +
                               '<button onclick="Display_None()">Annuler</button>';

        div_text.innerHTML = 'Voulez vous supprimer le message avec le signalement ?' + confirm_function;
   }
        }
    }
}
