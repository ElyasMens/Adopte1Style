...
function SaveProfile($connexion, $user_id, $username, $gender, $birthdate, $job, $user_location, $relation, $user_size, $user_weight, $eye_color, $hair_color, $biography, $style, $photo_for_profile){


    $query = "INSERT INTO users_info (id, username, gender, birthdate, job, user_location, relation, user_size, user_weight, eye_color, hair_color, biography, style, user_photo) 
                VALUES (:id, :username, :gender, :birthdate, :job, :user_location, :relation, :user_size, :user_weight, :eye_color, :hair_color, :biography, :style, :photo_for_profile)";
...
function UpdateProfile($connexion, $user_id, $username, $gender, $birthdate, $job, $user_location, $relation, $user_size, $user_weight, $eye_color, $hair_color, $biography, $style, $photo_for_profile) {


    $query = "UPDATE users_info SET gender = :gender, birthdate = :birthdate, job = :job, user_location = :user_location, relation = :relation, user_size = :user_size, user_weight = :user_weight, eye_color = :eye_color, hair_color = :hair_color, biography = :biography, style = :style, user_photo = :photo_for_profile
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($result){
        SaveProfile($connexion, $result['id'], $username, $sexe, $date_naissance, $profession, $lieu_residence, $situation_amoureuse, $taille, $poids, $couleur_yeux, $couleur_cheveux, $biographie, $style, $photo_for_profile);
    }else {
        $result2 = UserInfo_Exists($username, $connexion);
        if($result2){
            UpdateProfile($connexion, $result2['id'], $username, $sexe, $date_naissance, $profession, $lieu_residence, $situation_amoureuse, $taille, $poids, $couleur_yeux, $couleur_cheveux, $biographie, $style, $photo_for_profile);
        }      
    }
