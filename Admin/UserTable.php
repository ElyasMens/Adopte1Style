<?php 
    ...
    $stmt->execute();
    echo "<script src='../Assets/JavaScript/Recherche.js'></script>";
    echo "<ul>";
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $user_pp = pp_path($row['username']);
        echo "<li class='user-info show-user' data-id='{$row['id']}' data-realname='{$row['real_name']}' data-username='{$row['username']}' data-email='{$row['email']}'>
                ...
                <div class='user-info-option'>
                    ...
                    <span class='ban-option' onclick='confirm(\"{$row['username']}\", \"{$row['email']}\")'><span class='material-symbols-outlined'>person_remove</span></span>
                </div> 
            </li>";
    } 
    echo "</ul>";
