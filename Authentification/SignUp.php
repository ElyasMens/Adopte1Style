...
function emailUsed($email, $connexion){
...
function signUp($real_name, $email, $username, $password, $connexion) {
    if (emailBanned($email, $connexion)){
        // Email banni
        setcookie('ERROR_MESSAGE_SIGNUP','EMAIL BANNI.', time()+60,'/');
        header('Location: ../Index.php');
        exit();
    }
    if (emailUsed($email, $connexion)){
        // Email utilisé
        setcookie('ERROR_MESSAGE_SIGNUP','EMAIL DÉJA UTILISÉ.', time()+60,'/');
        header('Location: ../Index.php');
        exit();
    } 
    if (UserExists($username, $connexion)) {
        // Le nom d'utilisateur existe
        setcookie('ERROR_MESSAGE_SIGNUP','NOM D\'UTILISATEUR DÉJA UTILISÉ.', time()+60,'/');
        header('Location: ../Index.php');
        exit();
    }   
